...
serve(async (req) => {
  ...
  try {
    ...
    console.log('Converting image to anime style:', imageUrl);
    ...
    if (!animeImageUrl) {
      throw new Error('No anime image generated');
    }

    return new Response(
      JSON.stringify({ animeImageUrl }),
      { 
    ...
    console.error('Error in anime-convert function:', error);
